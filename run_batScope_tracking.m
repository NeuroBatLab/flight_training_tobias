function run_batScope_tracking(varargin)
...
pos
            pcounter = 0;
            while pos(1) > position_threshold %pos(1) < -2000 || pos(1) > 5000 %x-axis is the long axis between stands and feeders
                %do nothing
                frameOfData = mGetCurrentFrame();
                pos = round(mean(frameOfData.BodyData.Markers(2:4,:),2)); %3D position (x, y, z)
                pos
